...
* [通俗易懂设计模式](./article/design_pattern/design_pattern.md)
...
* redis和数据库双写一致性的解决方案
  ...
  crud中只有update需要保证双写一致性。可以用分布式事务把删redis和更新数据库放到一个事务中，事务期间其它修改线程和读取线程均不可以介入
  （强一致性）
...
* spring源码分析
  ...
  * [IOC容器启动流程源码解析](./article/spring_ioc_startup_sourcecode/spring_ioc_startup_sourcecode.md)
